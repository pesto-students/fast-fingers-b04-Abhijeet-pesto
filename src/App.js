...
export class App extends React.Component {
	...
	startGame = () => {
		if (!this.state.userName || this.state.userName.length === 0) {
			...
			const timerValue = Math.ceil(
				newWord.length / this.state.difficultyFactor,
			);
			this.setState({
	...
	restartGame = () => {
		const filteredWords = this.state.dictionary.filter(
			(word) => word.length <= 4,
		);
		const newWord =
			filteredWords[Math.floor(Math.random() * filteredWords.length)];
	...
	handleAnswerChange = (text) => {
		...
		if (text.toLowerCase() === this.state.currentWord.toLowerCase()) {
			...
			let newWordList = [];
			if (
				newDifficultyFactor >= 1.5 &&
				newDifficultyFactor < 2 &&
				this.state.difficultyLevel === 'EASY'
			) {
				newWordList = this.state.dictionary.filter(
					(word) => word.length >= 5 || word.length <= 8,
				);
			} else if (
				newDifficultyFactor >= 2 &&
				this.state.difficultyLevel === 'MEDIUM'
			) {
				newWordList = this.state.dictionary.filter((word) => word.length > 8);
			}
	...
	setNewWordandTimer = (factor, newList = []) => {
		...
		if (newList.length > 0) {
			...
			newWord = this.state.currentWordList[
				Math.floor(Math.random() * this.state.currentWordList.length)
			];
		}
